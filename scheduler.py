...
async def setup_custom_poll_times(job_queue, send_poll_func, chat_times):
    ...
    for chat_id, poll_time_str in chat_times.items():
        try:
            ...
            logger.info(f"Scheduled custom poll for chat {chat_id} at {poll_time_str} UTC")
...
async def daily_poll(context, send_poll_func):
    ...
    logger.info(f"Running daily poll for {len(registered_chats)} chats at {datetime.now()}")
...
async def custom_poll(context, send_poll_func, chat_id):
    ...
    if not poll_state.is_active(chat_id):
        await send_poll_func(chat_id, context, "Ah shit, here we go again! (Custom time poll)")
...
async def reschedule_poll_for_chat(job_queue, chat_id, send_poll_func):
    ...
    try:
        ...
        logger.info(f"Rescheduled custom poll for chat {chat_id} at {poll_time_str} UTC")
        return True
    except Exception as e:
