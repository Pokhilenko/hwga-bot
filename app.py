...
async def send_poll(context: ContextTypes.DEFAULT_TYPE):
    ...
    job_ping1 = scheduler.add_job(ping_unvoted, "date", run_date=datetime.datetime.now() + datetime.timedelta(minutes=10), args=[context])
    job_ping2 = scheduler.add_job(ping_unvoted, "date", run_date=datetime.datetime.now() + datetime.timedelta(minutes=20), args=[context])
    job_close = scheduler.add_job(close_poll, "date", run_date=datetime.datetime.now() + datetime.timedelta(minutes=20), args=[context])
...
async def get_known_user_ids(bot):
...
async def handle_poll_answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_user_ids = await get_known_user_ids(context.bot)
    if all_user_ids and all_user_ids.issubset(voted_user_ids):
        logger.info("Проголосовали все — закрываем опсос досрочно")
        await close_poll(context, early=True)


async def ping_unvoted(context: ContextTypes.DEFAULT_TYPE):
    all_user_ids = await get_known_user_ids(context.bot)
...
async def close_poll(context: ContextTypes.DEFAULT_TYPE, early=False):
    ...
    if len(voted) == len(all_user_ids) and all_user_ids:
        await context.bot.send_message(chat_id=CHAT_ID, text="Сасают все!")
    else:
...
async def main():
    ...
    scheduler.add_job(send_poll, "cron", hour=21, minute=0, args=[application.bot])
    scheduler.start()
